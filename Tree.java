...
public class Tree {
    ...
    public Node createTree(float[][] data, float[] labels) {
...
//                System.out.println("leaf" + p.isLeaf);
                maxDepth.add(child.depth);
//                System.out.println("depth" + child.depth);
                if(Entropy(child.data, child.getFeatureIndex(), -1, child.lables) > 0 && Entropy(child.data, child.getFeatureIndex(), -1, child.lables) < 1){
...
//        }
    ...
    public int findDepth(){
        int indexmax = maxDepth.get(0);
        for(int i = 1; i < maxDepth.size(); i++ ){
            if(maxDepth.get(i) >indexmax)
                indexmax = maxDepth.get(i);
        }
        return indexmax;
